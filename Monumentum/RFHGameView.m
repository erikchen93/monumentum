...
-(void)fetchFeed
{
    NSString *requestString = @"http://monumentum.herokuapp.com/words/list";
    NSURL *url = [NSURL URLWithString:requestString];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
    
    NSURLSessionDataTask *dataTask = [self.session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        NSDictionary *jsonObject = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
        self.set1 = jsonObject[@"words"];
        
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.set1Table reloadData];
        });
    }];
    NSLog(@"the session data task is suspended until this point, so the web request has not been made");
    [dataTask resume];
}
